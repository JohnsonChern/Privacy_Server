...
def list_extension (json_data):
    ...
    if isinstance(data_list['resourceType'], unicode):
	s=resource['resourceType']
    else:
	s=unicode(data_list['resourceType'],"utf-8")
...
def check_private_policy(resource, resource_id, client_id):
    ...
    elif resource['resourceType'] == 'Sequence':
        resource['text']['div']="<div> Not available to show due to privacy policy </div>"
	resource['coordinate']="Protected"
	resource = cover_protected_data(resource, resource, resource_id)
    elif resource['resourceType'] == 'Observation':
	resource['subject']['reference'] = "Unavialiable to see"
	resource = cover_protected_data(resource, resource, resource_id)
